...
- (void)configNavigationBar {
    UIBarButtonItem *leftItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"navigationButtonReturn"] style:UIBarButtonItemStylePlain target:self action:@selector(backRootViewController)];
    self.navigationItem.leftBarButtonItem = leftItem;
}
- (void)loadData
{
    NSString *path = [[NSBundle mainBundle] pathForResource:@"citydict" ofType:@"plist"];
    _hotCity = [NSArray arrayWithObjects:@"上海",@"北京",@"广州",@"深圳",@"武汉",@"天津",@"西安",@"南京",@"杭州",@"成都", nil];
    ...
    [_indexKeys insertObjects:@[@"定位",@"访问",@"热门"] atIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, 3)]];
